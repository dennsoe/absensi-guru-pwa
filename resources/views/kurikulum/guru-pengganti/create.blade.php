                ...
                <div class="card-body">
                    <form action="{{ route('kurikulum.guru-pengganti.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jadwal_mengajar_id" class="form-select @error('jadwal_mengajar_id') is-invalid @enderror" required id="jadwalSelect">
                                <option value="">Pilih Jadwal</option>
                                @foreach($jadwal_list as $j)
                                <option value="{{ $j->id }}" 
                                        data-guru-id="{{ $j->guru_id }}"
                            ...
                            @error('jadwal_mengajar_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="guruAsliDisplay" readonly placeholder="Pilih jadwal terlebih dahulu">
                            <input type="hidden" name="guru_asli_id" id="guruAsliId">
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Guru Pengganti <span class="text-danger">*</span></label>
                            <select name="guru_pengganti_id" class="form-select @error('guru_pengganti_id') is-invalid @enderror" required>
                            ...
                            @error('guru_pengganti_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
